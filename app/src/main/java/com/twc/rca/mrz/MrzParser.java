 ...
 */
package com.twc.rca.mrz;

import java.text.Normalizer;
import java.util.HashMap;
import java.util.Map;

import static com.twc.rca.mrz.MrzFormat.PASSPORT;
...
public class MrzParser {
    ...
    public String[] parseName(MrzRange range) {
        ...
        if (names.length == 1) {
            ...
            surname = parseString(new MrzRange(range.column, range.column + names[0].length(), range.row)).replace('1','I');
    ...
    public void checkValidCharacters(MrzRange range) {
        ...
        for (int i = 0; i < str.length(); i++) {
            ...
            if (c != FILLER && (c < '0' || c > '9') && (c < 'A' || c > 'Z') && (c < 'a' || c > 'z')) {
    ...
    public MrzDate parseDate(MrzRange range) {
        ...
        try {
            value=rawValue(r);
            year = Integer.parseInt(value.replace('S','5').replace('Z','2').replace('D','0'));
        ...
        try {
            value=rawValue(r);
            month = Integer.parseInt(value.replace('S','5').replace('Z','2').replace('D','0'));
        } catch (NumberFormatException ex) {
            month = -1;
        ...
        try {
            value=rawValue(r);
            day = Integer.parseInt(value.replace('S','5').replace('Z','2').replace('D','0'));
        } catch (NumberFormatException ex) {
            day = -1;
            // log.debug("Failed to parse MRZ date month " + rawValue(range) + ": " + ex, mrz, r);
        }
    ...
    public MrzSex parseSex(int col, int row) {
        ...
        if(Character.isLetter(c)) {
            if (c == 'H' || c == 'B')
                c = 'M';
            return MrzSex.fromMrz(c);
        }
            return MrzSex.fromMrz(' ');
    }
